...
def stk_name_v1(simpword):
    ...
    if simpword:
        mysql_read('select stockcode,stockname_std,stockname_simp from capvision_fun.dim_stkname_compname where stockname_simp !=""',tmpfn)
    dic = {}
    p_stk = '.*?('
...
def stkname_from_demand(isload,simpword):
    sql_fn = 'npt_cont_suglab_speciaql_.txt'    if simpword else 'npt_cont_suglab.txt'
    if isload: 
        if simpword :
            sql= 'truncate table demands_suglab_special;load data local infile "'+sql_fn+'" into table demands_suglab_special fields terminated by ";" ; '
            sql +='update capvision_fun.demands  dm join capvision_fun.demands_suglab_special sg on dm.id= sg.id set dm.special_astock_name = sg.stkname;'
        else:
            sql= 'truncate table demands_suglab;load data local infile "'+sql_fn+'" into table demands_suglab fields terminated by ";" ;'
            sql +='update capvision_fun.demands  dm join capvision_fun.demands_suglab sg on dm.id= sg.id set dm.sug_labels = sg.stkname; '

        os.system('mysql -uadmin -padmin2048az -h192.168.50.95 capvision_fun -e \''+ sql + '\'')
        return

    sql = 'SELECT id,date,demand from capvision_fun.demands where client_type ="MFund" and sug_labels ="" and date>"2016-12-01" order by id desc'
    mysql_read(sql,'/tmp/t1.txt')
    dic_stk, p_stk = stk_name_v1(simpword)
    #print p_stk
    OUTFN = open(sql_fn,'w')
    for line in open('/tmp/t1.txt'):
        id,date,demand = line.strip().split('<##>')
        nline = demand
        #print id,nline
        mat = re.match(p_stk, nline)
        stkcds, stknames = [], []
        while mat:
            stkname_key = mat.group(1)
            print stkname_key
            stkname = dic_stk[stkname_key][1]
            if stkname not in stknames:
                stknames.append(dic_stk[stkname_key][1])
                stkcds.append(dic_stk[stkname_key][0])
            nline = mat.group(2)
            mat = re.match(p_stk, nline)
        if stkcds:
            print >>OUTFN, '%s;%s;%s'%(id, ','.join(stkcds), ','.join(stknames))
...
def build_from_demand(demtag, outfn):
    ...
    if demtag == 'demand_req':
        sql = "select date,sug_labels,sug_lab_accuracy_mark from capvision_fun.demands where sug_labels !='' and client_type=='MFund';"  #sug_lab_accuracy_mark rlike '.*1' "
    ...
    for line in open(tmpfn):
        ...
        for ii in range(len(t_labels_accuracy_li)):            
            ...
            print >>OUTFN,'%s,%s,%s,%s,%s,%s'%(date, stkcd,stkname.decode('utf-8').encode('utf-8'),1,confirm_code,1)
...
ctype = sys.argv[1]
#if ctype == 'suglab':
if ctype == 'req_stkname':
    isload = sys.argv[2] == 'load'
    simpword = sys.argv[3] == 'simp'
    stkname_from_demand(isload,simpword)
if ctype == 'stk_proj':
